...
class StdController extends Controller
{
    ...
    public function paging(Request $request): JsonResponse
    {
        ...
        if (!isset($request->search)) {
            ...
            $todos = (new std())->get_data_($request->search, $arr_pagination);
        } else {
            $arr_pagination = (new PublicModel())->pagination_without_search(
                $URL,
                $request->limit,
        ...
        if ($request->yop) {
            $todos = $todos->where('yop', '=', $request->yop);
        }
        if ($request->mop) {
            $todos = $todos->where('mop', '=', $request->mop);
        }
        if ($request->distCode) {
        ...
        if ($request->onHandUnit) {
            $todos = $todos->where('onHandUnit', '=', $request->onHandUnit);
        }
    ...
    public function storeBulky(Request $req): JsonResponse
    {
        ...
        try {
            ...
            foreach ($data_csv as $key => $value) {
                ...
                $data['onHandUnit'] = $value['onHandUnit'];
                $data['created_by'] = $req->userid;
                $data['updated_by'] = $req->userid;
                $todos = std::create($data);
            }
            DB::commit();
            return response()->json([
                'code' => 201,
    ...
    //         $data = DB::table('stds')->get(); // sesuaikan dengan tabel Anda
    ...
    //     } catch (\Exception $e) {
    //         // Log error
    //         std::error('CSV Download Error: ' . $e->getMessage());
    ...
    public function pagingBulky(Request $request)
    {
        ...
        if ($request->has('export_csv') && $request->export_csv == true) {
            ...
            $data = std::when($request->search, function ($query) use ($request) {
                $search = $request->search;
                return $query->where('yop', 'like', "%$search%")
                    ->orWhere('mop', 'like', "%$search%")
                    ->orWhere('distCode', 'like', "%$search%")
                    ->orWhere('brchName', 'like', "%$search%")
                    ->orWhere('itemCode', 'like', "%$search%")
                    ->orWhere('onHandUnit', 'like', "%$search%")
                ;
            })->get();
        ...
        if (!isset($request->search)) {
            ...
            $todos = std::skip($offset)->take($limit)->get();
        } else {
            // Lakukan pagination dengan pencarian
            $search = $request->search;
            $todos = std::where('yop', 'like', "%$search%")
                ->orWhere('mop', 'like', "%$search%")
                ->orWhere('distCode', 'like', "%$search%")
                ->orWhere('brchName', 'like', "%$search%")
                ->orWhere('itemCode', 'like', "%$search%")
    ...
    public function destroy(Request $req, int $id): JsonResponse
    {
        ...
        try {
            $item = std::findOrFail($id);

            std::where('id', $id)->update(['deleted_by' => $user_id]);
