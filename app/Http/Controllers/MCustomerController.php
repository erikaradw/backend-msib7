...
class MCustomerController extends Controller
{
    ...
    {
        $this->judul_halaman_notif = 'MASTER CUSTOMER';
    }
    ...
    {
        try {
            $getAllData = M_Customer::with(["customer"])->get();
    ...
    {
        try {
            $todo = M_Customer::orderBy('dist_code', 'asc')->get();
            return response()->json([
                'code' => 200,
                'status' => true,
                'data' => $todo
    ...
    {
        try {
            $rowCount = DB::table('m__customers')->count();
            DB::table('m__customers')->truncate();

            Log::info('All data in m__customers table has been deleted.', ['row_count' => $rowCount]);
            ...
            Log::error('Failed to delete all data from m__customers table.', [
                'error' => $e->getMessage(),
    ...
    public function storeBulky(Request $req): JsonResponse
    {
        ...
        try {
            ...
            foreach ($data_csv as $key => $value) {
                ...
                $data['cust_name'] = $value['cust_name'];
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        try {
            $todo = M_Customer::findOrFail($id);

            M_Customer::where('id', $id)->update(['deleted_by' => $user_id]);
            $todo->delete();
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        try {
            ...
            M_Customer::where('id', $id)->update(['updated_by' => $user_id, 'updated_at' => date('Y-m-d H:i:s')]);
    ...
    {
        try {
            $todos = M_Customer::findOrFail($id);
            return response()->json([
                'code' => 200,
                'status' => true,
                'data' => $todos
